...
RSpec.describe User, type: :model do
  ...
  describe 'most_recent_posts' do
    ...
    it 'should return the three most recent posts and no more' do
      ...
      old_post = Post.create(title: 'first post', text: 'text', author_id: user.id)
      ...
      expect(user.most_recent_posts).to include(first_post)
      expect(user.most_recent_posts).to include(second_post)
      expect(user.most_recent_posts).to include(third_post)
      expect(user.most_recent_posts).to_not include(old_post)
    end
  end
end
