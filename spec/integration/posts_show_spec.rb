...
RSpec.describe 'User posts', type: :system, js: true do
  before(:all) do
    ...
    @cr7 = User.create(name: 'Cristiano', photo: 'https://picsum.photos/200/300',
  ...
  describe 'show page' do
    ...
    it 'should render a user\'s post information' do
      expect(page).to have_content(@messi_post1.title)
      expect(page).to have_content("by #{@messi.name}")
      expect(page).to have_content(@messi_post1.comments_counter)
      expect(page).to have_content(@messi_post1.likes_counter)
